...
export default function QuizPage() {
      ...
      <div className="grid gap-5">
        ...
        {actsLoading && !activities.length && (
          <div className="mx-auto grid w-full max-w-2xl gap-2">
            {[0, 1, 2].map((i) => (
              <div key={i} className="flex items-center gap-3 rounded-2xl border border-slate-200/80 dark:border-[var(--yrk-border-subtle)] bg-white dark:bg-[var(--yrk-surface-elevated)] p-3">
        ...
        <Card><CardHeader><CardTitle>Configure your round</CardTitle><CardDescription>Segment, size and pace.</CardDescription></CardHeader><CardContent className="grid gap-4">
          ...
          {activityId && <div className="flex items-center gap-2 text-[13px]"><span className="rounded-full bg-brand-50 px-3 py-1 font-semibold text-brand-700">Activity: {activityTitle || activityId}</span><button className="text-slate-400 dark:text-[var(--yrk-text-tertiary)] underline" onClick={() => { setActivityId(""); setActivityTitle(""); }}>clear</button></div>}
          {setId && <div className="flex items-center gap-2 text-[13px]"><span className="rounded-full bg-brand-50 px-3 py-1 font-semibold text-brand-700">Exam set: {setTitle || setId}</span><button className="text-slate-400 dark:text-[var(--yrk-text-tertiary)] underline" onClick={() => { setSetId(""); setSetTitle(""); }}>clear</button></div>}
          {!sourceId && <>
          ...
          {subjectId && <div className="flex items-center gap-2 text-[13px]"><span className="rounded-full bg-brand-50 px-3 py-1 font-semibold text-brand-700">Subject: {subjectName || subjectId}</span><button className="text-slate-400 dark:text-[var(--yrk-text-tertiary)] underline" onClick={() => { setSubjectId(""); setSubjectName(""); }}>clear</button></div>}
          </>}
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="flex flex-wrap gap-2 p-3">
            ...
            {!!result.wrongIds.length && (
              <button onClick={() => setWrongOnly((v) => !v)} className={`ml-auto rounded-lg px-3 py-1.5 text-[13px] font-semibold transition ${wrongOnly ? "bg-slate-900 text-white" : "bg-slate-100 dark:bg-white/[0.07] text-slate-500 dark:text-[#9aa3b2] hover:text-slate-800 dark:text-[var(--yrk-text-primary)]"}`}>
              ...
              <CardContent className="yrk-wrap grid gap-2 p-4 text-sm">
                ...
                {b.parts?.length ? (
                  <div className="grid gap-1">
                    {b.parts.map((p, j) => (
                      <div key={j} className={`flex flex-wrap items-center gap-x-2 rounded-lg px-2.5 py-1.5 text-[13px] ${p.ok ? "bg-green-50 dark:bg-green-950 text-green-900 dark:text-green-200" : "bg-red-50 dark:bg-red-950 text-red-900 dark:text-red-200"}`}>
                        <span className="font-medium">{p.stem}:</span>
                        <span>You: {p.given.join(", ") || "(blank)"}</span>
                        <span className="text-slate-500 dark:text-[#9aa3b2]">· Expected: {p.expected.join(", ")}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-slate-500 dark:text-[#9aa3b2]">{isT ? `Your answer: ${b.given.join(" ").slice(0, 200) || "(blank)"}` : `You: ${b.given.join(", ") || "(blank)"} · Answer: ${b.correctAnswers.join(", ")}`}</div>
                )}
                <div className="rounded-lg bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] px-2.5 py-2 text-[13px]">{isT ? `Marking guide: ${b.explanation}` : b.explanation}</div>
              </CardContent>
            </Card>
          );
        })}
  ...
  return (
    <div className="grid gap-3">
      {/* runner top bar */}
      <div className="sticky top-16 z-20 overflow-hidden rounded-2xl border border-slate-200 dark:border-[var(--yrk-border-subtle)] bg-white/95 shadow-soft backdrop-blur">
        <div className="flex items-center gap-2 px-3 py-2">
          <button title="Exit round" onClick={exitRound} className="rounded-lg p-1.5 text-slate-400 dark:text-[var(--yrk-text-tertiary)] transition hover:bg-slate-100 dark:hover:bg-white/[0.07] hover:text-slate-700 dark:text-[#c6ccd6]"><X className="h-4 w-4" /></button>
            <span className="min-w-0 flex-1 truncate text-[13px] font-semibold">
              {MODE_LABEL[mode]} <span className="font-normal text-slate-400 dark:text-[var(--yrk-text-tertiary)]">· Q{idx + 1}/{items.length} · {(cur.type ?? "").replace(/_/g, " ")} · {cur.difficulty}</span>
            </span>
          {flagged[cur.id] && <span className="flex shrink-0 items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-bold text-amber-800 dark:text-amber-200"><Flag className="h-3 w-3" />Flagged</span>}
          {mode !== "practice" && (
            <span className={`flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[13px] font-bold tabular-nums ${lowTime ? "bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300" : "bg-slate-900 text-white"}`}>
          ...
          <button title="Questions" onClick={() => setShowPalette((v) => !v)}
            className={`rounded-lg p-1.5 transition lg:hidden ${showPalette ? "bg-slate-900 text-white" : "text-slate-500 dark:text-[#9aa3b2] hover:bg-slate-100 dark:hover:bg-white/[0.07]"}`}>
            <LayoutGrid className="h-4 w-4" />
          </button>
        </div>
        <div className="h-1 bg-slate-100 dark:bg-white/[0.07]"><div className="h-full bg-slate-900 transition-all" style={{ width: `${progress}%` }} /></div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="grid gap-2">
            {!isFill && !isTheory && !isRubric && !PARTS_Q.includes(cur.type) && cur.type !== "saq" && cur.type !== "sct" && opts.map((o, oi) => {
              ...
              return (
                <button key={o} onClick={() => toggleOpt(o)}
                  className={`flex min-w-0 items-center gap-3 rounded-xl border px-3 py-3 text-left text-[15px] transition active:scale-[0.995] ${sel ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 dark:border-[var(--yrk-border-subtle)] bg-white dark:bg-[var(--yrk-surface-elevated)] hover:border-slate-400 dark:hover:border-[var(--yrk-border-strong)]"} ${show && right ? "!border-green-600 !bg-green-50 dark:bg-green-950 !text-green-900 dark:text-green-200" : ""} ${show && sel && !right ? "!border-red-600 !bg-red-50 dark:bg-red-950 !text-red-900 dark:text-red-200" : ""}`}>
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[13px] font-black ${sel ? "bg-white/20" : show && right ? "bg-green-600 text-white" : "bg-slate-100 dark:bg-white/[0.07] text-slate-500 dark:text-[#9aa3b2]"}`}>{letter(oi)}</span>
            ...
            {cur.type === "mtf" && parts.map((p, i) => (
              <div key={i} className="flex min-w-0 items-center justify-between gap-2 rounded-xl border border-slate-200 dark:border-[var(--yrk-border-subtle)] px-3.5 py-2.5 text-sm">
                  ...
                  {(["True", "False"] as const).map((v) => {
                    ...
                    return (
                      <button key={v} onClick={() => setPart(i, v)}
                        className={`rounded-lg border px-3 py-1.5 text-[13px] font-semibold transition ${sel ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 dark:border-[var(--yrk-border-subtle)] hover:border-slate-400 dark:hover:border-[var(--yrk-border-strong)]"} ${show && right ? "!border-green-600 !bg-green-50 dark:bg-green-950 !text-green-900 dark:text-green-200" : ""} ${show && sel && !right ? "!border-red-600 !bg-red-50 dark:bg-red-950 !text-red-900 dark:text-red-200" : ""}`}>{v}</button>
                    );
                  })}
                </span>
              </div>
            ...
            {cur.type === "meq" && parts.slice(0, meqShown[cur.id] ?? 1).map((p, i) => (
              <label key={i} className="grid gap-1 rounded-xl bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-sm">
                <span className="font-medium">Step {i + 1}. {p.stem}</span>
                <Input className="bg-white dark:bg-[var(--yrk-surface-elevated)]" placeholder="Your answer…" value={picked[i] ?? ""} onChange={(e) => setPart(i, e.target.value)} />
              </label>
            ))}
            ...
            {isRubric && (
              <div className="grid gap-2">
                <div className="text-[13px] text-slate-500 dark:text-[#9aa3b2]">Score each criterion honestly — this is self/peer assessment.</div>
                {parts.map((p, i) => (
                  <div key={i} className="flex items-center justify-between gap-2 rounded-xl border border-slate-200 dark:border-[var(--yrk-border-subtle)] px-3.5 py-2.5 text-sm">
                    <span className="min-w-0 break-words">{p.label} <span className="text-slate-400 dark:text-[var(--yrk-text-tertiary)]">· max {p.max}</span></span>
                    <span className="flex shrink-0 items-center gap-1.5">
                      <button className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 dark:border-[var(--yrk-border-subtle)] text-base hover:bg-slate-50 dark:hover:bg-white/[0.05] dark:bg-[var(--yrk-surface-canvas)] sm:h-7 sm:w-7" onClick={() => setCriterion(i, (rScores[i] ?? 0) - 1, Number(p.max) || 0)}>−</button>
            ...
            {isFill && (
              <div className="grid gap-2 rounded-xl bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-[15px] leading-loose">
                  ...
                  <span key={i}>
                    ...
                    {i < arr.length - 1 && (
                      <input value={picked[i] ?? ""} onChange={(e) => setGap(i, e.target.value)}
                        placeholder={`gap ${i + 1}`}
                        className="mx-1 inline-block w-28 max-w-full rounded-lg border border-slate-300 dark:border-[var(--yrk-border-default)] bg-white dark:bg-[var(--yrk-surface-elevated)] px-2 py-1.5 text-sm shadow-sm focus:border-brand-500 focus:outline-none sm:w-36" />
                    )}
                  </span>
                ))}
            ...
            {isTheory && (
              <textarea className="min-h-[120px] w-full rounded-md border border-slate-200 dark:border-[var(--yrk-border-subtle)] p-3 text-sm" placeholder={cur.type === "essay" ? "Write your essay answer…" : "Write a concise answer…"} value={picked[0] ?? ""} onChange={(e) => setText(e.target.value)} />
            )}
            {showInstant && picked.length > 0 && !isFill && !isTheory && !isRubric && !PARTS_Q.includes(cur.type) && cur.type !== "saq" && cur.type !== "sct" && (
              <div className="rounded-lg bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-sm">{instantOk ? "Correct. " : `Answer: ${correct.join(", ")}. `}{cur.explanation}</div>
            )}
            {showInstant && (allPartsFilled || (cur.type === "saq" && picked[0]) || (cur.type === "sct" && picked[0])) && (
              <div className="rounded-lg bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-sm"><span className="font-medium">{instantOk ? "Correct. " : cur.type === "saq" ? `Accepted: ${correct.join(" / ")}. ` : ""}</span>{cur.explanation}</div>
            )}
            {showInstant && isRubric && (
              <div className="rounded-lg bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-sm"><span className="font-medium">Marking guide, compare: </span>{cur.explanation}</div>
            )}
            {showInstant && ((isFill && picked.length > 0 && picked[0]) || (isTheory && picked.length > 0 && picked[0])) && (
              <div className="rounded-lg bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-3 text-sm"><span className="font-medium">{isTheory ? "Marking guide, compare: " : ""}</span>{cur.explanation}</div>
            )}
            <div className="sticky-safe sticky bottom-3 z-10 mt-1 flex items-center gap-1.5 rounded-2xl border border-slate-200 dark:border-[var(--yrk-border-subtle)] bg-white/95 px-2 py-2 shadow-lift backdrop-blur">
              ...
              <span className="min-w-0 flex-1 truncate text-center text-xs tabular-nums text-slate-400 dark:text-[var(--yrk-text-tertiary)]">{idx + 1} / {items.length}</span>
              {idx < items.length - 1
            ...
            {items.map((q, i) => {
              ...
              return (
                <button key={q.id} onClick={() => { setIdx(i); setShowPalette(false); }} title={q.stem}
                  className={`flex h-10 w-10 items-center justify-center rounded-lg border text-xs font-semibold transition sm:h-8 sm:w-8 ${i === idx ? "border-slate-900 bg-slate-900 text-white" : done ? "border-green-600 bg-green-50 dark:bg-green-950 text-green-900 dark:text-green-200" : "border-slate-200 dark:border-[var(--yrk-border-subtle)] bg-white dark:bg-[var(--yrk-surface-elevated)] hover:border-slate-400 dark:hover:border-[var(--yrk-border-strong)]"} ${flagged[q.id] ? "ring-2 ring-amber-400" : ""}`}>{i + 1}</button>
              );
            })}
          </div>
          <div className="flex flex-wrap gap-x-3 gap-y-1 border-t border-slate-100 dark:border-[var(--yrk-border-subtle)] pt-2 text-[11px] text-slate-400 dark:text-[var(--yrk-text-tertiary)]">
...
function ActivityPicker({ activities, mode }: {
  ...
  const bannerBg: Record<string, string> = {
    indigo: "bg-brand-500", emerald: "bg-emerald-500", amber: "bg-amber-500",
  ...
  return (
      <div className="mx-auto grid w-full max-w-2xl gap-4">
        <h2 className="text-sm font-bold uppercase tracking-[0.12em] text-slate-400 dark:text-[var(--yrk-text-tertiary)]">Curriculum · {activities.length} activit{activities.length === 1 ? "y" : "ies"}</h2>
      {groups.map((g) => (
          ...
          <div className="grid gap-2">
            {g.items.map((a) => (
              <a key={a.id} href={`/archive/${a.id}?mode=${mode}`}
                className="flex items-center gap-3 rounded-2xl border border-slate-200/80 dark:border-[var(--yrk-border-subtle)] bg-white dark:bg-[var(--yrk-surface-elevated)] p-3 shadow-soft transition hover:shadow-lift">
                <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-lg font-black text-white ${bannerBg[a.banner] ?? "bg-brand-500"}`}>
                  {(a.title.trim().charAt(0) || "A").toUpperCase()}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-bold">{a.title}</span>
                  <span className="block truncate text-xs text-slate-400 dark:text-[var(--yrk-text-tertiary)]">by {a.ownerName} · {a.questionCount} Qs{a.sector ? ` · ${a.sector}` : ""}{a.visibility === "public" ? " · public" : ""}</span>
                </span>
...
function MatchingInput({ parts, options, picked, correct, onAssign }: {
  ...
  return (
    <div className="grid gap-2.5">
      <div className="flex flex-wrap gap-1.5 rounded-xl bg-slate-50 dark:bg-[var(--yrk-surface-canvas)] p-2.5">
        {pool.length ? pool.map((o) => (
          <span key={o} draggable onDragStart={(e) => { e.dataTransfer.setData("text/plain", o); e.dataTransfer.effectAllowed = "move"; }}
            className="flex cursor-grab items-center gap-1 rounded-lg border border-brand-200 bg-white dark:bg-[var(--yrk-surface-elevated)] px-2.5 py-1.5 text-[13px] font-medium text-brand-800 shadow-sm active:cursor-grabbing">
            <GripVertical className="h-3.5 w-3.5 text-slate-300" />{o}
          </span>
        )) : <span className="text-[13px] text-slate-400 dark:text-[var(--yrk-text-tertiary)]">All choices placed — drag one back here to unassign (or clear its select).</span>}
      </div>
      {parts.map((p, i) => {
        ...
        return (
          <div key={i} onDragOver={(e) => e.preventDefault()} onDrop={onDropZone(i)}
            className={`grid gap-1.5 rounded-xl border-2 border-dashed p-2.5 transition ${show ? (right ? "border-green-400 bg-green-50/50" : "border-red-300 bg-red-50/50") : "border-slate-200 dark:border-[var(--yrk-border-subtle)]"}`}>
            <span className="text-sm font-medium">{i + 1}. {p.stem}</span>
            <div className="flex items-center gap-2">
              <span className={`min-w-24 flex-1 rounded-lg border px-2.5 py-1.5 text-[13px] ${assigned[i] ? "border-brand-300 bg-brand-50 font-semibold text-brand-900" : "border-slate-200 dark:border-[var(--yrk-border-subtle)] text-slate-400 dark:text-[var(--yrk-text-tertiary)]"}`}>
                {assigned[i] || "Drop a choice here…"}
              </span>
              {assigned[i] && <button className="rounded-md px-1.5 py-1 text-slate-400 dark:text-[var(--yrk-text-tertiary)] hover:bg-slate-100 dark:hover:bg-white/[0.07] hover:text-slate-700 dark:text-[#c6ccd6]" title="Clear" onClick={() => onAssign(i, "")}>×</button>}
            </div>
