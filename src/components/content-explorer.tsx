  ...
  function checkRow(kind: "q" | "f", id: string, name: string, sub: string, extra?: Partial<PickedRef>) {
    ...
    return (
      <button onClick={() => toggle({ kind, id, name, sub, ...extra })}
        className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-md border text-xs font-bold transition sm:h-6 sm:w-6", on ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 dark:border-[var(--yrk-border-default)] bg-white dark:bg-[var(--yrk-surface-elevated)] text-transparent hover:border-slate-500")}>✓</button>
    );
  }

  return (
    <div className="yrk-sheet fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-3 sm:p-6" onClick={onClose}>
      <div className="flex max-h-[85dvh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white dark:bg-[var(--yrk-surface-elevated)] shadow-lift" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-[var(--yrk-border-subtle)] px-5 py-3.5">
        ...
        <div className="flex gap-1 border-b border-slate-100 dark:border-[var(--yrk-border-subtle)] px-4 py-2">
          {(["mine", "shared", "public"] as Tab[]).map((t) => (
            <button key={t} onClick={() => reset(t)}
              className={cn("rounded-lg px-3 py-1.5 text-[13px] font-semibold transition", tab === t ? "bg-slate-900 text-white" : "text-slate-500 dark:text-[#9aa3b2] hover:bg-slate-100 dark:hover:bg-white/[0.07]")}>
              {t === "mine" ? "My bank" : t === "shared" ? "Shared with me" : "Public folders"}
            </button>
          ))}
          <div className="relative ml-auto w-44">
            <Search className="absolute left-2.5 top-2 h-3.5 w-3.5 text-slate-400 dark:text-[var(--yrk-text-tertiary)]" />
            <Input className="h-8 pl-8 text-[13px]" placeholder="Filter…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        </div>
        {crumbs.length > 1 && (
          <nav className="flex items-center gap-1 overflow-x-auto border-b border-slate-100 dark:border-[var(--yrk-border-subtle)] px-5 py-2 text-[13px]">
            {crumbs.map((c, i) => (
              <span key={i} className="flex shrink-0 items-center gap-1">
                {i > 0 && <ChevronRight className="h-3.5 w-3.5 text-slate-300" />}
                <button onClick={() => { setFolderId(c.id); setOwnerId(c.ownerId ?? null); }}
                  className={i === crumbs.length - 1 ? "font-semibold" : "text-brand-600 hover:underline"}>{c.name}</button>
              </span>
            ))}
          </nav>
        )}
        <div className="grid flex-1 gap-1 overflow-y-auto p-3">
          {atRoot && (tab === "shared" || tab === "public") && showRoots.map((r) => (
            <div key={`${r.ownerId}:${r.id}`} className="flex items-center gap-2.5 rounded-xl px-2 py-2 hover:bg-slate-50 dark:hover:bg-white/[0.05] dark:bg-[var(--yrk-surface-canvas)]">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-50 dark:bg-amber-950"><Folder className="h-4 w-4 text-amber-500" fill="#fcd34d" /></span>
              <button className="min-w-0 flex-1 text-left" onClick={() => enterRoot(r)}>
                <span className="block truncate text-sm font-semibold">{r.name}</span>
                <span className="block truncate text-xs text-slate-400 dark:text-[var(--yrk-text-tertiary)]">{r.ownerName} · {r.role === "public-use" ? "public, usable" : r.role}</span>
              </button>
              {r.id && tab === "public" && checkRow("f", r.id, r.name, `Public · ${r.ownerName}`)}
            </div>
          ))}
          {showFolders.map((f) => (
            <div key={f.id} className="flex items-center gap-2.5 rounded-xl px-2 py-2 hover:bg-slate-50 dark:hover:bg-white/[0.05] dark:bg-[var(--yrk-surface-canvas)]">
              ...
              <button className="min-w-0 flex-1 text-left" onClick={() => enterFolder(f.id)}>
                ...
                <span className="block truncate text-xs text-slate-400 dark:text-[var(--yrk-text-tertiary)]">Folder · tap to open</span>
              </button>
            </div>
          ))}
          {showFiles.map((f) => {
          ...
          {!showRoots.length && !showFolders.length && !showFiles.length && (
            <div className="px-2 py-8 text-center text-sm text-slate-400 dark:text-[var(--yrk-text-tertiary)]">
              {atRoot && tab !== "mine" ? "Nothing here yet." : "Empty folder — open one or check elsewhere."}
            </div>
          )}
        </div>
        <div className="flex items-center gap-2 border-t border-slate-100 dark:border-[var(--yrk-border-subtle)] px-4 py-3">
          <span className="min-w-0 flex-1 truncate text-[13px] text-slate-500 dark:text-[#9aa3b2]">
