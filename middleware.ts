...
const PROTECTED = [/^\/workspaces(\/.*)?$/, /^\/profile$/, /^\/notifications$/, /^\/admin(\/.*)?$/, /^\/bank\/subject\/[^/]+\/contribute$/];
...
export const config = {
  matcher: ["/workspaces/:path*", "/profile", "/notifications", "/admin/:path*", "/bank/subject/:id/contribute"]
};
