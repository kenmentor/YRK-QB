...
export default function PublicFolderPage({ params }: { params: { id: string } }) {
  ...
  if (gone) return <div className="grid gap-3 py-10 text-center"><div className="flex items-center justify-center gap-2 font-bold"><Lock className="h-4 w-4" /> This folder isn't public.</div><a href="/archive" className="text-sm text-brand-600 underline">Back to archive</a></div>;
  ...
  return (
    <div className="grid gap-4">
      ...
      <Card><CardContent className="flex flex-wrap items-center gap-2 p-4">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-950"><Folder className="h-4 w-4 text-emerald-600" fill="#6ee7b7" /></span>
        <div className="min-w-0 flex-1">
          <div className="truncate text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 dark:text-[var(--yrk-text-tertiary)]">Public folder · by {ownerName}</div>
          <nav className="flex flex-wrap items-center gap-1 text-sm">
            {trail.map((c, i) => (
              <span key={i} className="flex items-center gap-1">
                {i > 0 && <span className="text-slate-300">›</span>}
                <button onClick={() => c.id && setFid(c.id)} className={i === trail.length - 1 ? "font-bold" : "text-brand-600 hover:underline"}>{c.name}</button>
              </span>
            ))}
          </nav>
        </div>
