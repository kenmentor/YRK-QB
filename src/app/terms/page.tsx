    ...
    <div className="mx-auto grid max-w-2xl gap-5">
      ...
      <Card><CardHeader><CardTitle>What you agree to</CardTitle></CardHeader>
        <CardContent className="grid gap-2.5 text-[15px]">
          {TERMS.map((t, i) => (
            <div key={i} className="flex gap-3 rounded-2xl border border-slate-100 px-4 py-3">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-50 text-[13px] font-bold text-indigo-700">{i + 1}</span>
              <span className="leading-relaxed text-slate-600">{t}</span>
            </div>
          ))}
          <div className="mt-2 flex gap-2">
            <a href="/bank"><Button>Browse the bank</Button></a>
            <a href="/register"><Button variant="secondary">Create account</Button></a>
          </div>
        </CardContent>
      </Card>
    </div>
